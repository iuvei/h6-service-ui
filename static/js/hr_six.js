...
function updateAnimal6Odds(){
	...
	if(window.top.lotteryData.pkStatus == OPEN_STATUS){
		...
		$(".ctrlPanel .ctrlCont .animal6Ctrl .odds:eq(1)").attr('data-creditplaytypeid', window.top.rateData[1141002][2]);
...
function setAnimalLinkCount(count, updateOdds){
	...
	$("#animalLinkPanel .animalLinkCell .radio.selected").removeClass("selected");
	$("#animalLinkPanel .animalLinkCell .animalRadio" + groupCount).addClass("selected");
	var name = $("#animalLinkPanel .animalLinkCell .animalRadio" + groupCount).next().text()
	var miss = $("#animalLinkPanel .animalMissTypeCell .selected").next().text() == '中' ? '' : $("#animalLinkPanel .animalMissTypeCell .selected").next().text()
	window.top.lotteryData.rate.forEach(item => {
		if (item.creditPlayId == localStorage.getItem('creditPlayId')) {
			item.creditPlayTypeDtoList.forEach(s => {
				if (s.creditPlayInfoName == (name + miss)) {
					sessionStorage.setItem('creditPlayInfoId', s.creditPlayInfoId)
				}
			})
		}
...
function setAnimalLinkIsMiss(isMiss, updateOdds){
	...
	var name = $("#animalLinkPanel .animalLinkCell .selected").next().text()
	var miss = $("#animalLinkPanel .animalMissTypeCell .selected").next().text() == '中' ? '' : $("#animalLinkPanel .animalMissTypeCell .selected").next().text()
	window.top.lotteryData.rate.forEach(item => {
		if (item.creditPlayId == localStorage.getItem('creditPlayId')) {
			item.creditPlayTypeDtoList.forEach(s => {
...
function setUnitLinkCount(count, updateOdds){
	...
	var name = $("#unitLinkPanel .unitLinkCell .unitRadio" + groupCount).next().text()
	var miss = $("#unitLinkPanel .unitMissTypeCell .selected").next().text() == '中' ? '' : $("#unitLinkPanel .unitMissTypeCell .selected").next().text()
	window.top.lotteryData.rate.forEach(item => {
		if (item.creditPlayId == localStorage.getItem('creditPlayId')) {
			item.creditPlayTypeDtoList.forEach(s => {
				if (s.creditPlayInfoName == (name + miss)) {
...
function bet(panelId){
	...
	for(var i = 0; i < betMoneyValueArr.length; i++){
		...
		if (panelId === 'sAnimalPanel') {
			...
			var pEle = betMoneyValueArr.eq(i).parents('.cell')
	...
	Send(httpUrlData.generalBet, JSON.stringify(data), function(obj){
		...
		data.forEach(function(item) {
			if (['特肖', '半波'].includes(gameType)) {
				...
				var str = ['特码', '正码'].includes(gameType) ? gameType : localStorage.getItem('creditPlayName')
				result.push(str + ' ' + item.panKou + '盘 ' + item.ballNum + '@' + item.rate + '=' + item.commandLogAmount + ' OK')
			}
		})
...
function betLinkNormalPanel(){
	...
	for(var i = 0; i < selectedBallArr.length; i++){
		...
		contentArr.push(selectedBallArr.eq(i).text()[0] == '0' ? selectedBallArr.eq(i).text()[1] : selectedBallArr.eq(i).text())
	}
	contentArr.sort(function(a, b){return a-b})
	data.push({
		"gameId": localStorage.getItem('gameId') || 1,
		"gamePeriodId": window.top.lotteryData.issue,
		"creditPlayId": sessionStorage.getItem('creditPlayInfoId'),
...
function betLinkHeadPanel(){
	...
	var ele = $('.linkNumBtnBox').find('.btn.curBtn').text()
	console.log(ele)
	var data = []
	var a = []
	var b = []
	...
	if(linkMode == 2 && headSelectedBallArr.length < linkCount - 1)
		...
		headSelectedBallArr.each(function() {
			a.push($(this).text()[0] == '0' ? $(this).text()[1] : $(this).text())
		})
		badySelectedBallArr.each(function() {
			b.push($(this).text()[0] == '0' ? $(this).text()[1] : $(this).text())
		})
		...
		data.push({
			...
			"creditPlayId": sessionStorage.getItem('creditPlayInfoId'),
			"creditPlayTypeId": null,
			"content": a.toString() + (linkMode == 2 ? '拖' : '碰') + b.toString(),
			"panKou": null,
			"commandLogAmount": parseInt(linkBetMoney)
		})
		betData = data
...
function betLinkAnimalPanel(){
	...
	if(selectedCheckArr.length < 2 || isNaN(linkBetMoney))
		...
		data.push({
			...
			"creditPlayId": sessionStorage.getItem('creditPlayInfoId'),
			"creditPlayTypeId": null,
			"content": $('#linkAnimalPanel .linkNum').text(),
			"panKou": null,
			"commandLogAmount": parseInt(linkBetMoney)
		})
		betData = data
...
function betLinkUnitNumPanel(){
	...
	data.push({
		...
		"creditPlayId": sessionStorage.getItem('creditPlayInfoId'),
		"creditPlayTypeId": null,
		"content": $('#linkUnitNumPanel .linkNum').text(),
		"panKou": null,
		"commandLogAmount": parseInt(linkBetMoney)
	})
	betData = data
...
function betLinkMixturePanel(){
	...
	data.push({
		...
		"creditPlayId": sessionStorage.getItem('creditPlayInfoId'),
		"creditPlayTypeId": null,
		"content": $('#linkMixturePanel .linkNum').text(),
		"panKou": null,
		"commandLogAmount": parseInt(linkBetMoney)
	})
	betData = data
...
function betMissPanel(){
	...
	for(var i = 0; i < selectedArr.length; i++){
		...
		ar.push(selectedArr.eq(i).attr("info")[0] == '0' ? selectedArr.eq(i).attr("info")[1] : selectedArr.eq(i).attr("info"));
	}
	numArr.sort();
